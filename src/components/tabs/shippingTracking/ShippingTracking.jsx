...
const SHIPPING_STATUSES = [
    ...
    { id: 'SHIPPED', label: 'Shipped', color: 'bg-yellow-100 border-yellow-300', textColor: 'text-yellow-800' },
    { id: 'ARRIVED', label: 'Arrived', color: 'bg-purple-100 border-purple-300', textColor: 'text-purple-800' },
    { id: 'CLEARED', label: 'Cleared', color: 'bg-blue-100 border-blue-300', textColor: 'text-blue-800' },
    { id: 'DELIVERED', label: 'Delivered', color: 'bg-green-100 border-green-300', textColor: 'text-green-800' }
];
...
const ShippingTracking = () => {
    ...
    const shippingCounts = useSelector(selectShippingCounts);
    ...
    const fetchInitialVehicles = async () => {
        try {
            ...
            response.data.forEach(vehicle => {
                const status = vehicle.vehicle_shipping?.shipping_status || 'PROCESSING';
                if (grouped[status]) {
    ...
    const onDragEnd = async (result) => {
        ...
        try {
            await dispatch(updateVehicleShipping({
                ...
                shippingData: {
                    ...removed.vehicle_shipping,
                    shipping_status: destination.droppableId
                }
            })).unwrap();
            ...
            // Revert on error
            setColumns(columns);
            showNotification('error', 'Error', 'Failed to update shipping status: ' + error.message);
        }
    };
                                ...
                                <div className={`rounded-lg ${status.color} border-2 p-3 mb-3`}>
                                    <div className="flex items-center justify-between">
                                        <h3 className={`font-semibold ${status.textColor}`}>
                                            {status.label} ({shippingCounts[status.id] || 0})
                                        </h3>
