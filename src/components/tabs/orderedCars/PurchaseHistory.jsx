...
import { X, ShoppingCart, Calendar, Building2, User, Clock, ArrowRight, CreditCard, DollarSign, TrendingUp, AlertCircle, CheckCircle, FileText } from 'lucide-react';
...
const PurchaseHistory = ({ vehicleId, onClose }) => {
    ...
    const getStatusConfig = (status) => {
        const configs = {
            'LC_PENDING': {
                ...
                icon: Clock,
                label: 'LC Pending',
                description: 'Awaiting LC processing'
            },
            'LC_OPENED': {
                color: 'bg-blue-100 text-blue-800 border-blue-300',
                icon: FileText,
                label: 'LC Opened',
                description: 'Letter of Credit opened'
            },
            'LC_RECEIVED': {
                color: 'bg-green-100 text-green-800 border-green-300',
                icon: CheckCircle,
                label: 'LC Received',
                description: 'LC documents received'
            },
            'CANCELLED': {
    ...
    const calculateStats = () => {
        ...
        const totalCost = history.reduce((sum, entry) => sum + (entry.lc_cost_jpy || 0), 0);

        return {
            totalChanges: history.length,
            avgDuration: formatDuration(avgDuration),
            totalCost: totalCost > 0 ? formatCurrency(totalCost) : null,
    ...
    return (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-5xl max-h-[90vh] flex flex-col">
                {/* Header */}
                <div className="bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 px-6 py-5 flex-shrink-0 rounded-t-xl">
                    ...
                    <div className="px-6 py-4 bg-gradient-to-r from-indigo-50 to-purple-50 border-b border-indigo-100">
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            <div className="bg-white rounded-lg p-3 shadow-sm">
                                ...
                                <p className="text-2xl font-bold text-indigo-600">{stats.totalChanges}</p>
                            </div>
                            <div className="bg-white rounded-lg p-3 shadow-sm">
                                <div className="flex items-center space-x-2 mb-1">
                                ...
                                <p className="text-lg font-bold text-purple-600">{stats.avgDuration}</p>
                            </div>
                            {stats.totalCost && (
                                <div className="bg-white rounded-lg p-3 shadow-sm">
                                    <div className="flex items-center space-x-2 mb-1">
                                        <DollarSign className="w-4 h-4 text-green-600" />
                                        <p className="text-xs font-medium text-gray-600">Total LC Cost</p>
                                    </div>
                                    <p className="text-lg font-bold text-green-600">{stats.totalCost}</p>
                                </div>
                            )}
                            <div className="bg-white rounded-lg p-3 shadow-sm">
                                <div className="flex items-center space-x-2 mb-1">
                                ...
                                <p className="text-sm font-bold text-blue-600">{getStatusConfig(stats.currentStatus).label}</p>
                            </div>
                        </div>
                    </div>
                )}
                    ...
                    {loading ? (
                        <div className="flex items-center justify-center py-16">
                            <div className="text-center">
                                <div className="relative">
                                    ...
                                    <ShoppingCart className="w-6 h-6 text-indigo-600 absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2" />
                                </div>
                                <p className="text-gray-600 mt-4 font-medium">Loading purchase history...</p>
                            </div>
                        </div>
                    ) : error ? (
                        ...
                        <div className="text-center py-16">
                            ...
                            <p className="text-gray-600 max-w-md mx-auto">
                                No status changes have been recorded yet. Purchase history will appear here once the vehicle goes through the purchasing process.
                            </p>
                        </div>
                    ) : (
                        <div className="relative">
                            {/* Timeline line */}
                            <div className="absolute left-10 top-0 bottom-0 w-0.5 bg-gradient-to-b from-indigo-300 via-purple-300 to-pink-300"></div>
                                ...
                                {history.map((entry, index) => {
                                    ...
                                    return (
                                        <div key={entry.id} className="relative pl-20">
                                            ...
                                            <div className="absolute left-8 -ml-3 w-6 h-6 rounded-full border-4 border-white shadow-lg flex items-center justify-center bg-gradient-to-br from-indigo-500 to-purple-500">
                                                {index === 0 && (
                                                        ...
                                                        {entry.supplier_id && (
                                                            <div className="flex items-start space-x-3 p-3 bg-gray-50 rounded-lg">
                                                                ...
                                                                <div className="flex-1 min-w-0">
                                                                    <p className="text-xs font-medium text-gray-500 mb-0.5">Supplier</p>
                                                                    <p className="text-sm font-semibold text-gray-900">ID: {entry.supplier_id}</p>
                                                                </div>
                                                            </div>
                                                        )}
                                                        {entry.lc_bank && (
                                                            <div className="flex items-start space-x-3 p-3 bg-gray-50 rounded-lg">
                                                                ...
                                                                <div className="flex-1 min-w-0">
                                                                    <p className="text-xs font-medium text-gray-500 mb-0.5">LC Bank</p>
                                                                    <p className="text-sm font-semibold text-gray-900">{entry.lc_bank}</p>
                                                                </div>
                                                            </div>
                                                        )}
                                                        {entry.lc_number && (
                                                            <div className="flex items-start space-x-3 p-3 bg-gray-50 rounded-lg">
                                                                ...
                                                                <div className="flex-1 min-w-0">
                                                                    <p className="text-xs font-medium text-gray-500 mb-0.5">LC Number</p>
                                                                    <p className="text-sm font-semibold text-gray-900">{entry.lc_number}</p>
                                                                </div>
                                                            </div>
                                                        )}
                                                        {entry.lc_cost_jpy && (
                                                            <div className="flex items-start space-x-3 p-3 bg-gradient-to-br from-green-50 to-emerald-50 rounded-lg border border-green-200">
                                                                ...
                                                                <div className="flex-1 min-w-0">
                                                                    <p className="text-xs font-medium text-gray-500 mb-0.5">LC Cost</p>
                                                                    <p className="text-lg font-bold text-green-600">{formatCurrency(entry.lc_cost_jpy)}</p>
                                                                </div>
                                                            </div>
                                                        )}
                                                    </div>
                                                    ...
                                                    {entry.purchase_date && (
                                                        <div className="mb-4">
                                                            <div className="flex items-center space-x-2 p-3 bg-blue-50 rounded-lg border border-blue-200">
                                                                ...
                                                                <div>
                                                                    <span className="text-xs font-medium text-blue-600">Purchase Date: </span>
                                                                    <span className="text-sm font-semibold text-blue-900">{formatDate(entry.purchase_date)}</span>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    )}
